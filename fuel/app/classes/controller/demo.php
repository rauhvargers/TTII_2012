...
class Controller_Demo extends Controller_Template {
	...
	$this->template->page_content = View::forge('welcome/helloworld', $viewdata);
	...
	$inputform->add("location", "Select a location", array(
	    "type" => "select",
	    "name" => "location_id",
	    "options" =>
	...
	$inputform->add("submit", "", array("type" => "submit",
	    "value" => "Add",
	...
	$form->add("lastname", "Input your surname");
	...
	$events = Model_Orm_Event::find("all", array('related' => array('agendas',
			'location'))
	);
    ...
    /**
     * Renders a form for file uploads. The form is later submitted to post_upload
     * @return view HTML data
     */
    public function get_upload() {
	return Response::forge(View::forge("demo/upload"));
    }

    /**
     * Receives a file the user has uploaded
     * Redirects:
     *	- to first page on success
     *	- to upload form if data is not valid
     */
    public function post_upload() {
	//no "old files" exist, let's catch the new ones!
	$config = array(
	    'path' => DOCROOT . 'user_uploads',
	    'randomize' => true,
	    'auto_rename' => true
	);
	...
	if (Upload::is_valid()) {
	    ...
	    //no uploaded files back to upload form!
	    return Response::redirect("demo/upload");
